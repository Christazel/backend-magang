...
// 1. Security HTTP Headers (XSS filter, nosniff, frameguard, HSTS, dll)
app.use(helmet());

// 2. Request Logging — aktif kecuali saat test
if (process.env.NODE_ENV !== "test") {
  app.use(morgan("dev"));
}

// 3. Global Rate Limiter — anti spam/DDoS untuk semua endpoint
const globalLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 menit
  max: 500, // maks 500 request per IP per 15 menit
...
app.use(mongoSanitize()); // Cegah NoSQL Injection ($, . pada query MongoDB)
app.use(xss()); // Cegah XSS pada input user
